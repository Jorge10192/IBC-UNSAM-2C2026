...
def evaluacion_online_OLS(X: np.ndarray, Y: np.ndarray) -> np.ndarray:
    ...
    """
    log_evidencias = np.zeros(D)

    for grado in range(D):

        # Arrancamos solamente con el primer dato
        modelo = OLS(
            Y[:1],
            phi(X[:1], grado)
        ).fit()
        ...
        for i in range(1, len(X)):
            ...
            # 1. PREDICCIÓN
            # -----------------------------
            h = modelo.params.values

            prob = prediccion_puntual(
                y=float(Y[i, 0]),
                x=float(X[i, 0]),
                h=h
            )
            ...
            log_evidencias[grado] += np.log(prob)
...
def calcular_log_evidencias(X: np.ndarray, Y: np.ndarray) -> np.ndarray:
    ...
    for grado in range(D):
        ...
        Phi = phi(X, grado).values

        log_ev = ml.log_evidence(
            Y,
            Phi,
            alpha=ALPHA,
            beta=BETA
        ...
        log_evidencias[grado] = float(np.asarray(log_ev).squeeze())
...
def calcular_posterior_modelos(X: np.ndarray, Y: np.ndarray) -> np.ndarray:
    ...
    """
    log_evidencias = calcular_log_evidencias(X, Y)

    # Priors iguales: P(M) = 1/D
    log_prior = np.log(1 / D)

    log_posterior_no_norm = log_evidencias + log_prior

    # Estabilidad numérica
    log_posterior_no_norm -= np.max(log_posterior_no_norm)

    posterior_no_norm = np.exp(log_posterior_no_norm)

    posterior = posterior_no_norm / np.sum(posterior_no_norm)
...
def construir_modelo_base(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    altura_madre = df["altura_madre"].to_numpy(dtype=float)
    ...
    Y = df["altura"].to_numpy(dtype=float).reshape(-1, 1)
...
def construir_modelo_identitario(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
    ...
    cantidad_grupos = int(df["id"].max() / 2)

    # Según la definición de la guía:
    # ID mod (max(ID)/2)
    grupos = ids % cantidad_grupos

    indicadores = np.column_stack([
        (grupos == grupo).astype(float)
...
def comparar_modelos_altura(df: pd.DataFrame) -> dict:
    ...
    """
    # ---------------------------------------------------------
    # 1. Construir los tres modelos
    # ---------------------------------------------------------

    PHI_base, Y = construir_modelo_base(df)

    PHI_biologico, _ = construir_modelo_biologico(df)

    PHI_identitario, _ = construir_modelo_identitario(df)

    # ---------------------------------------------------------
    # 2. Calcular log-evidencias
    # ---------------------------------------------------------

    evidencia_base = float(
        np.asarray(
            ml.log_evidence(
                Y,
                PHI_base,
    ...
    log_evidencias = np.array([
        evidencia_base,
        evidencia_biologico,
        evidencia_identitario
    ])
    ...
    # ---------------------------------------------------------
    # 4. Prior uniforme sobre modelos
    # ---------------------------------------------------------

    log_prior = np.log(1 / 3)

    log_scores = log_evidencias + log_prior
    ...
    posterior_modelos = {
        "Base": posterior[0],
        "Biológico": posterior[1],
        "Identitario": posterior[2]
    }

    # Modelo con mayor evidencia
    mejor_modelo = nombres[np.argmax(log_evidencias)]

    return {
        "evidencia_base": evidencia_base,
        "evidencia_biologico": evidencia_biologico,
        "evidencia_identitario": evidencia_identitario,
